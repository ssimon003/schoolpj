...
```console
helm install my-release oci://REGISTRY_NAME/REPOSITORY_NAME/nginx --set "global.imagePullSecrets[0]=SECRET_NAME" -n NAMESPACE
```
...
| `metrics.securityContext.enabled`                  | Enabled NGINX Exporter containers' Security Context                                                                                                                                                                               | `true`                           |
...
| `metrics.securityContext.runAsGroup`               | Set NGINX Exporter container's Security Context runAsGroup                                                                                                                                                                        | `1001`                           |
| `metrics.securityContext.runAsNonRoot`             | Set NGINX Exporter container's Security Context runAsNonRoot                                                                                                                                                                      | `true`                           |
| `metrics.securityContext.privileged`               | Set NGINX Exporter container's Security Context privileged                                                                                                                                                                        | `false`                          |
| `metrics.securityContext.readOnlyRootFilesystem`   | Set NGINX Exporter container's Security Context readOnlyRootFilesystem                                                                                                                                                            | `true`                           |
| `metrics.securityContext.allowPrivilegeEscalation` | Set NGINX Exporter container's Security Context allowPrivilegeEscalation                                                                                                                                                          | `false`                          |
| `metrics.securityContext.capabilities.drop`        | List of capabilities to be dropped                                                                                                                                                                                                | `["ALL"]`                        |
| `metrics.securityContext.seccompProfile.type`      | Set NGINX Exporter container's Security Context seccomp profile                                                                                                                                                                   | `RuntimeDefault`                 |
...
The Prometheus metrics port (9113) has been removed from the main nginx Service and is now served exclusively by a new `<release>-metrics` ClusterIP Service. Any existing scrape configuration that targeted the main Service — including `prometheus.io/scrape`-annotated discovery, hand-crafted `PodMonitor` or `ServiceMonitor` resources, or static Prometheus job configs pointing at the main service's port 9113 — **will stop receiving metrics after upgrading** and must be updated to point at the `<release>-metrics` Service instead.

If you use the bundled `ServiceMonitor` (`metrics.serviceMonitor.enabled=true`), no action is needed: the ServiceMonitor is automatically updated to select the new `<release>-metrics` Service.
